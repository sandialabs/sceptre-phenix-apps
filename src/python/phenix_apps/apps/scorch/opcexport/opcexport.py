...
class OPCExport(ComponentBase):
        ...
        if elastic_ip != "172.16.0.254":
            ...
            if tap_ip:  # only configure if one of the tap apps was found
                ...
                utils.mm_exec_wait(
                    self.mm,
                    host,
                    f"route add {target_subnet} MASK 255.255.255.0 {tap_ip}",
                )
    ...
    def start(self):
        ...
        self.mm.cc_background_once(
            f"C:/Progra~1/Python38/python.exe C:/opcexport/scada_to_elastic.py -u opc.tcp://{host}:4840 -f /opcexport/opc_variables.json -e {elastic_ip}:{elastic_port}"
        )
    ...
    def cleanup(self):
        ...
        utils.mm_exec_wait(
            self.mm,
            host,
            f"C:/Progra~1/Python38/python.exe C:/opcexport/delete_scada_to_elastic.py -e {elastic_ip}:{elastic_port}",
        )
    ...
    def create_opc_variables(self, opc_config_file: str) -> dict:
        ...
        for elem in root.iter("{http://www.kepware.com/schemas/servermain}Channel"):
            channel_name = elem.find(
                "{http://www.kepware.com/schemas/servermain}Name"
            ).text
            device_list = elem.find(
                "{http://www.kepware.com/schemas/servermain}DeviceList"
            ).findall("{http://www.kepware.com/schemas/servermain}Device")
            for device in device_list:
                device_name = device.find(
                    "{http://www.kepware.com/schemas/servermain}Name"
                ).text
                device_tag_list = []
                tag_list = device.find(
                    "{http://www.kepware.com/schemas/servermain}TagList"
                ).findall("{http://www.kepware.com/schemas/servermain}Tag")
                for tag in tag_list:
                    tag_name = tag.find(
                        "{http://www.kepware.com/schemas/servermain}Name"
                    ).text
