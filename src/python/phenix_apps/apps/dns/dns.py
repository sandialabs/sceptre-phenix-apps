...
class DNS(AppBase):
    def __init__(self, name: str, stage: str, dryrun: bool = False) -> None:
        ...
        # Create directories: /phenix/experiments/<exp-name>/{startup, dns}/
    ...
    def configure(self):
        if not (dns_nodes := self.extract_labelled_topology_nodes("dns-server")):
            logger.error(
                "No DNS server defined, expected at least one topology node with 'dns-server' label"
            )
            sys.exit(1)
        ...
        if not (
            dns_ip_mask := self.extract_node_interface_ip(
                hostname, dns_iface, include_mask=True
            )
        ):
            logger.error(
                f"Failed to find interface '{dns_iface}' for DNS server '{hostname}'"
            )
            sys.exit(1)
        ...
        for node in self.experiment.spec.topology.nodes:
            ...
            for i in node.network.interfaces:
                ...
                if (
                    self.metadata.get("same_subnet", False)
                    and ip_address(i.address) not in dns_net
                ):
                    continue
                ...
                if i.address in hosts:
                    logger.warning(
                        f"duplicate IP {i.address} from host {node.general.hostname}"
                    )
        ...
        stop_path = self.render(
            "stop_resolved.sh.mako", f"{self.startup_dir}/70_stop_resolved.sh"
        )
        ...
        start_path = self.render(
            "dnsmasq_start.sh.mako", f"{self.startup_dir}/71_dnsmasq_start.sh"
        )
        ...
        # /etc/resolv.conf
        resolv_path = self.render("resolv.conf.mako", f"{self.dns_dir}/resolv.conf")
        self.add_inject(
            hostname,
            {
                "src": resolv_path,
