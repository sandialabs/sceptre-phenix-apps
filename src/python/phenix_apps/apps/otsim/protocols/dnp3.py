    ...
    def registers_to_xml(self, registers):
        ...
        for reg in registers:
            if reg.type == "analog-read":
                input_el = ET.SubElement(parent, "input", {"type": "analog"})

                addr = ET.SubElement(input_el, "address")
                addr.text = str(self.addrs["ai"])

                self.addrs["ai"] += 1

                tag = ET.SubElement(input_el, "tag")
                tag.text = reg.tag

                if "sgvar" in reg.md:
                    svar = ET.SubElement(input_el, "sgvar")
                    svar.text = reg.md["sgvar"]

                if "egvar" in reg.md:
                    evar = ET.SubElement(input_el, "egvar")
                    evar.text = reg.md["egvar"]

                if "class" in reg.md:
                    klass = ET.SubElement(input_el, "class")
                ...
                input_el = ET.SubElement(parent, "input", {"type": "binary"})
