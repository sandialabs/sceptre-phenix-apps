'''
This is a "dependacy free" implementaion of a register-map genrator, only using
native python modules. This module simple takes in a field device config and
genrates an XML file that can be read my Microsoft Excel."
'''
...
import json
class RegMapGen():
    def __init__(cls, exp_dir, exp,filename="excel.xml", num_cols="11", num_rows="591"):
        wb_attrib = {"xmlns":"urn:schemas-microsoft-com:office:spreadsheet",
        ...
        cls.dp = ET.SubElement(cls.workbook, 'DocumentProperties',attrib={"xmlns":"urn:schemas-microsoft-com:office:office"})
        ...
        cls.ods = ET.SubElement(cls.workbook, 'OfficeDocumentSettings',attrib={"xmlns":"urn:schemas-microsoft-com:office:office"})
        ...
        #init ExcelWorkbook
        cls.ew = ET.SubElement(cls.workbook, 'ExcelWorkbook',attrib={"xmlns":"urn:schemas-microsoft-com:office:excel"})
        ...
        item = ET.SubElement(cls.styles, 'Style',attrib={"ss:ID":"Default", "ss:Name":"Normal"})
        ET.SubElement(item, 'Font', attrib={"ss:FontName":"Arial","x:CharSet":"1"})

        #Style: timestamp
        style = ET.SubElement(cls.styles, 'Style',attrib={"ss:ID":"created"})
        ET.SubElement(style, 'Alignment',attrib={"ss:Horizontal":"Right", "ss:Vertical":"Center"})
        ET.SubElement(style, 'Borders')
        ET.SubElement(style, 'Font',attrib={"ss:FontName":"calibri", "x:CharSet":"1", "ss:Color":"#000000" ,"ss:Italic":"1"})
        ...
        #Style: index_bar
        cls.add_style("index_bar","#808080",{"ss:FontName":"calibri", "x:CharSet":"1", "ss:Color":"#FFFFFF", "ss:Bold":"1","ss:Italic":"1"})
        #Style: small_dark
        cls.add_style("small_dark","#C0C0C0",{"ss:FontName":"calibri", "x:CharSet":"1", "ss:Color":"#000000"})
        #Style: small_light
        cls.add_style("small_light","#EBEBEB",{"ss:FontName":"calibri", "x:CharSet":"1", "ss:Color":"#000000"})
        #Style: ip_proto_dark
        cls.add_style("ip_proto_dark","#C0C0C0",{"ss:FontName":"calibri", "x:CharSet":"1", "ss:Color":"#000000"})
        #Style: ip_proto_light
        cls.add_style("ip_proto_light","#EBEBEB",{"ss:FontName":"calibri", "x:CharSet":"1", "ss:Color":"#000000"})
        #Style: field_device_dark
        cls.add_style("field_device_dark","#C0C0C0",{"ss:FontName":"calibri", "x:CharSet":"1", "ss:Color":"#000000","ss:Bold":"1","ss:Italic":"1"})
        #Style: field_device_light
        cls.add_style("field_device_light","#EBEBEB",{"ss:FontName":"calibri", "x:CharSet":"1", "ss:Color":"#000000","ss:Bold":"1","ss:Italic":"1"})

        #init Worksheet
        cls.worksheet = ET.SubElement(cls.workbook,"Worksheet",attrib={"ss:Name":"Register Map"})

        #init table
        cls.table = ET.SubElement(cls.worksheet,"Table",attrib={"ss:ExpandedColumnCount":num_cols, "ss:ExpandedRowCount":num_rows, "x:FullColumns":"1","x:FullRows":"1", "ss:DefaultColumnWidth":"130", "ss:DefaultRowHeight":"13"})
        row = ET.SubElement(cls.table,"Row",attrib={"ss:Height":"14"})
        cls.add_cell(row,"created","String","Created:")
        cls.add_cell(row,"date","DateTime",datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S'))
        ...
        #worksheet cont.
        cls.wo = ET.SubElement(cls.worksheet,"WorksheetOptions",attrib={'xmlns':"urn:schemas-microsoft-com:office:excel"})
        cls.ps = ET.SubElement(cls.wo,"PageSetup")
        ET.SubElement(cls.ps,"Layout", attrib={ "x:CenterHorizontal":"1", "x:StartPageNumber":"1"})
        ET.SubElement(cls.ps,"Header", attrib={"x:Margin":"0.1" ,"x:Data":"&amp;P"})
        ET.SubElement(cls.ps,"Footer", attrib={"x:Margin":"0.1" ,"x:Data":"&amp;F"})
        ET.SubElement(cls.ps,"PageMargins",attrib={"x:Bottom":"0.37", "x:Left":"0.3", "x:Right":"0.3", "x:Top":"0.61"})
    ...
    def add_style(cls,sid,int_color,fnt_dict):
        ...
        ET.SubElement(style, "Alignment", attrib={"ss:Horizontal":"Center", "ss:Vertical":"Center"})
        bds = ET.SubElement(style, "Borders", )
        ET.SubElement(bds, "Border", attrib={"ss:Position":"Bottom", "ss:LineStyle":"Continuous", "ss:Weight":"1",
     "ss:Color":"#FFFFFF"})
        ET.SubElement(bds, "Border", attrib={"ss:Position":"Left", "ss:LineStyle":"Continuous", "ss:Weight":"1",
     "ss:Color":"#FFFFFF"})
        ET.SubElement(bds, "Border", attrib={"ss:Position":"Right", "ss:LineStyle":"Continuous", "ss:Weight":"1",
     "ss:Color":"#FFFFFF"})
        ET.SubElement(style, "Font", attrib=fnt_dict)
        ET.SubElement(style, "Interior", attrib={"ss:Color":int_color, "ss:Pattern":"Solid"})
        ET.SubElement(style, "Protection")
    ...
    def new_row(cls,regs,i,proto=[],dev=[],ip=""):
        if i ==0 and dev != []:
            row = ET.SubElement(cls.table,"Row",attrib={"ss:Height":"14"})
            if cls.fd_bit:
                cls.add_cell(row,"field_device_dark","String",dev[0],md=str(dev[1]),index="2")
                cls.fd_bit = not cls.fd_bit
            else:
                cls.add_cell(row,"field_device_light","String",dev[0],md=str(dev[1]),index="2")
                cls.fd_bit = not cls.fd_bit

            if cls.ip_bit:
                cls.add_cell(row,"ip_proto_dark","String",ip,md=str(dev[1]))
                cls.add_cell(row,"ip_proto_light","String",proto[0],md=str(proto[1]))
            ...
            if cls.ip_bit:
                cls.add_cell(row,"ip_proto_dark","String",proto[0],md=str(proto[1]),index="4")
                cls.ip_bit = not cls.ip_bit
            else:
                cls.add_cell(row,"ip_proto_light","String",proto[0],md=str(proto[1]),index="4")
                cls.ip_bit = not cls.ip_bit
...
def generate_file(fd_configs, exp_dir, exp):
    ...
    for name, field_dev  in fd_configs.items():
        ...
        ## Generate a map containg protocols and their associated registers
        for devname, regs in  field_dev.registers.items():
            for reg in regs:
                ...
                if reg.protocol in protos:
                    protos[reg.protocol].append({"device":devname,"device_type":reg.devtype,"register_number":reg.addr, "rw":reg.fieldtype,"scale":scale,"data":reg.field,"register_type":reg.regtype})
                else:
                    protos[reg.protocol] = []
                    protos[reg.protocol].append({"device":devname,"device_type":reg.devtype,"register_number":reg.addr, "rw":reg.fieldtype,"scale":scale,"data":reg.field,"register_type":reg.regtype})
            ...
            for reg in sorted(regs,key=lambda k:k['register_number']):
                ...
                if i ==0:
                    if len(protos_names) == len(protos.items()):
                        wb.new_row(reg,i,[key.upper(),(len(protos[key])-1)],[name.upper(),(total_reg_cnt-1)],ip=fd_addr)
                    else :
...
def update_config(config,fd,devname,protocol,fieldtype,field,register_number,regtype):
            ...
            for j in range(len(config["nodes"][i]["metadata"][protocol])):
                if config["nodes"][i]["metadata"][protocol][j]["name"] == devname:
                    if fieldtype in config["nodes"][i]["metadata"][protocol][j]:
                        config["nodes"][i]["metadata"][protocol][j][fieldtype].append({"field":field,"register_number":register_number,"register_type":regtype})
                    else:
        ...
        for proto in devconfig.protocols:
            for dev in proto.devices:
                for reg in dev.registers:
                    config = update_config(config,devconfig.name,reg.devname,proto.protocol,reg.fieldtype,reg.field,reg.addr,reg.regtype)
