...
class RegMapGen:
    def __init__(cls, exp_dir, exp, num_cols="11", num_rows="591"):
        wb_attrib = {
    ...
    def new_row(cls, regs, i, proto=None, dev=None, ip=""):
        if dev is None:
            dev = []
        if proto is None:
            proto = []
...
def generate_json(topology, topo_dir, fd_configs, experiment_directory):
    ...
    with open(f"{experiment_directory}/ConfigWithCustomRegs.json", "w") as f:
