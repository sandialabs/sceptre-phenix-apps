...
class SunSpecDevice:
    ...
    def __init__(self, infra, devname, registers):
        ...
        if self.infra is None: self.infra = 'PowerDistribution'
    ...
    def generate_registers(self, models):
        ...
        for model_id in models:
            ...
            model = ET.ElementTree(file=f'{base_path}/protocols/sunspec/models/smdx/{xml_file}').getroot()
            model = model.find('model')

            register = SunSpecDevice.Register(self.infra, self.devname, model.get('name'), 'uint16', model.get('id'))
            self.registers.append(register)

            register = SunSpecDevice.Register(self.infra, self.devname, 'length', 'uint16', model.get('len'))
            self.registers.append(register)
            ...
            for block in blocks: # model 126 has multiple block elements defined
                for point in block:
                    ...
                    field     = None
                    scaling   = point.get('sf') # Will return `None` if point doesn't have a scaling factor
                    ...
                    if model_id == 1: # common block -- set some static values
                        if name == 'Mn':
                            ...
                            field = ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(32))

                    register = SunSpecDevice.Register(self.infra, self.devname, name, fieldtype, field, scaling)
                    self.registers.append(register)


    class Register:
        sizes = {
            'uint16': 1, 'uint32': 2, 'acc32': 2, 'acc64': 4, 'bitfield16': 1,
            'bitfield32': 2, 'enum16': 1, 'int16': 1, 'int32': 2, 'string8': 8,
            'string16': 16, 'sunssf': 1, 'float32': 2, 'pad': 1,
        }

        disabled = {
            'uint16': 65535, 'uint32': 4294967295, 'acc32': 0, 'acc64': 0,
            'bitfield16': 65535, 'bitfield32': 4294967295, 'enum16': 65535,
            'int16': 32767, 'int32': 2147483648, 'string8': 0, 'string16': 0,
            'sunssf': 0, 'float32': 2147483648, 'pad': 0,
        }

        mappings = { # array index 0 indicates static value
            'PowerDistribution': {
                'A': [False, 'current_mag_total'], 'AphA': [False, 'current_mag_p1'],
                'AphB': [False, 'current_mag_p2'], 'AphC': [False, 'current_mag_p3'],
                'PhVphA': [False, 'voltage_mag_p1'], 'PhVphB': [False, 'voltage_mag_p2'],
                'PhVphC': [False, 'voltage_mag_p3'], 'W': [False, 'real_power_total'],
                'Hz': [True, '600'], 'WH': [True, '0'], 'Conn': [False, 'active'],
                'WMaxLimPct': [True, '100'], 'WMaxLim_Ena': [True, '0'],
                'OutPFSet': [True, '100'], 'OutPFSet_Ena': [True, '0'], 'VarPct_Ena': [True, '0'],
                'ActCrv': [True, '1'], 'ModEna': [False, 'volt_var_enable'], 'NCrv': [True, '1'],
                'NPt': [True, '6'], 'ActPt': [True, '6'], 'DeptRef': [False, 'volt_var_reference'],
                'V1': [False, 'curve_volt_pt_1'], 'VAr1': [False, 'curve_var_pt_1'],
                'V2': [False, 'curve_volt_pt_2'], 'VAr2': [False, 'curve_var_pt_2'],
                'V3': [False, 'curve_volt_pt_3'], 'VAr3': [False, 'curve_var_pt_3'],
                'V4': [False, 'curve_volt_pt_4'], 'VAr4': [False, 'curve_var_pt_4'],
                'V5': [False, 'curve_volt_pt_5'], 'VAr5': [False, 'curve_var_pt_5'],
                'V6': [False, 'curve_volt_pt_6'], 'VAr6': [False, 'curve_var_pt_6'],
                'ReadOnly': [True, '0'],
            },
            'PowerTransmission': { # setting all these values using logic
                'A': [True, '0'], 'AphA': [True, '0'],
                'AphB': [True, '0'], 'AphC': [True, '0'],
                'PhVphA': [True, '0'], 'PhVphB': [True, '0'],
                'PhVphC': [True, '0'], 'W': [True, '0'],
                'Hz': [True, '0'], 'Conn': [True, '0'],
                'WMaxLimPct': [True, '0'], 'WMaxLim_Ena': [True, '0'],
                'WRtg': [True, '0'], 'VArMaxPct': [True, '0'],
                'VArRtg': [True, '0'], 'VAr': [True, '0'],
                'ReadOnly': [True, '0'],
            },
        }

        scalings = {
            'A_SF': '-2', 'V_SF': '-2', 'VAr_SF': '-2', 'W_SF': '-2', 'Hz_SF': '-1', 'WH_SF': '0',
            'Tmp_SF': '-1', 'WMaxLimPct_SF': '-2', 'OutPFSet_SF': '-2', 'DeptRef_SF': '-1',
            'WRtg_SF': '0', 'VArPct_SF': '-2',
        }
        ...
        def __init__(self, infra, devname, name, fieldtype, field = None, scaling = None):
            ...
            if field is not None:
                ...
                if self.name in mapping:
                    ...
                    if fieldtype == 'pad': self.fieldtype = 'int16'
