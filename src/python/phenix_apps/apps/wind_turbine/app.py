...
class WindTurbine(AppBase):
            ...
            for node in extracted:
                ...
                if match and len(match.groups()) > 0:
                    ...
                    for i in range(len(groups)):
                        ...
                        md.controllers.anemometer = md.controllers.anemometer.replace(
                            f"${group}", groups[i]
                        )
                        md.controllers.yaw = md.controllers.yaw.replace(
                            f"${group}", groups[i]
                        )

                        for j in range(len(md.controllers.blades)):
                            blade = md.controllers.blades[j].replace(
                                f"${group}", groups[i]
                            )
                        ...
                        if (
                            ...
                            for name, value in (
                                md["ground-truth-module"]["elastic"]
                                .get("labels", {})
                                .items()
                            ):
                                md["ground-truth-module"]["elastic"]["labels"][name] = (
                                    value.replace(f"${group}", groups[i])
                                )
                    ...
                    anemo = Box(
                        {
                            ...
                            "metadata": {
                                "type": "anemometer",
                                "template": md.get("template", "default"),
                                "ground-truth-module": md.get(
                                    "ground-truth-module", {}
                                ),
                            },
                    ...
                    for blade in md.controllers.blades:
                        self.__blade_controller(
                            blade,
                            {"ground-truth-module": md.get("ground-truth-module", {})},
                        )
                else:
    ...
    def __main_controller(self, node, anemo=None, yaw=None, match=None):
        ...
        tmpl = self.templates.get(node.metadata.get("template", "default"), {}).get(
            "main-controller", {}
        )
    ...
    """
        ...
        registers = [
            Register(
                "binary-read-write", "turbine.emergency-stop", {"sbo": str(sbo).lower()}
            ),
    ...
    def __blade_controller(self, hostname, md=None):
        if md is None:
            md = {}
        node = Box(
            {
