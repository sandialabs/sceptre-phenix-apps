...
| `count` | Primary scaling factor. Meaning depends on the plugin (e.g., number of VMs or number of assets). |
...
| `container_template` | Configuration for the workload inside the VM (e.g., `rootfs`, `networks`, `gateway`). |
...
# Post-Start Stage
phenix-app-scale post-start --dry-run < phenix_apps/apps/scale/tests/test_scale_input.yaml
```