...
class Scale(AppBase):
    def __init__(self, name: str, stage: str, dryrun: bool = False) -> None:
        ...
        self.app_dir = f"{self.exp_dir}/{self.name}"
    ...
    def _discover_plugins(self) -> None:
            ...
            for ep in eps:
                if ep.name.startswith("phenix-scale-plugin-"):
                    try:
                        ep.load()
                        logger.log(
                            "DEBUG", f"Loaded plugin from console script: {ep.name}"
                        )
                    except Exception as e:
                        logger.log(
                            "ERROR", f"Failed to load plugin from script {ep.name}: {e}"
                        )
        except Exception as e:
            logger.log("WARNING", f"Failed to discover plugins via entry_points: {e}")
        ...
        for name in plugins_to_load:
            ...
            try:
                ...
                logger.log("DEBUG", f"Loaded internal plugin: {name}")
            except ImportError as e:
                logger.log("ERROR", f"Failed to load plugin '{name}': {e}")
                continue
            except Exception as e:
                logger.log(
                    "ERROR",
    ...
    def configure(self) -> None:
        ...
        logger.log(
            "INFO", f"Configuring user app: {self.name} with {len(profiles)} profiles"
        )
        ...
        for profile in profiles:
            ...
            logger.log(
                "INFO", f"Processing profile '{p_name}' with plugin '{p_plugin}'"
            )
            ...
            if config_dump:
                logger.log("DEBUG", f"Plugin '{p_plugin}' configuration: {config_dump}")

            node_count = plugin.get_node_count()
            logger.log("INFO", f"Profile '{p_name}': Scaling {node_count} nodes.")
        ...
        logger.log(
            "INFO", f"Configured user app: {self.name} with {len(profiles)} profiles"
        )
    ...
    def _configure_node_common(
        ...
        if additional_cmds:
            startup_script_content += additional_cmds + "\n"
        startup_script_content += (
            "while [ ! -S /tmp/minimega/minimega ]; do sleep 1; done\n"
        )
        startup_script_content += f"while [ ! -f {mm_file} ]; do sleep 1; done\n"
        startup_script_content += f"ovs-vsctl add-br {self.exp_name}\n"
        startup_script_content += f"ovs-vsctl add-port {self.exp_name} ens1\n"
        startup_script_content += f"mm read {mm_file}\n"
        startup_script_content += "echo 'DONE!'\n"
        ...
        for net in networks:
            ...
            try:
                ...
                if self.dryrun:
                    ...
                    continue
                logger.log("WARN", f"VLAN not found: {net['name']}")
                continue
    ...
    def _get_gateway(self, gateway: Optional[str]) -> Optional[str]:
        ...
        try:
            ...
            if not tap_app:
                if self.dryrun:
                    return "192.168.1.1"
                logger.log(
                    "ERROR", "Tap app not found! Required for gateway resolution."
                )
                return None
            ...
            logger.log("ERROR", "VLAN specified for gateway is not tapped")
            return None
