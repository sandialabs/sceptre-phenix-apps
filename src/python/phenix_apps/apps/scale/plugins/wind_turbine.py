...
class WindTurbineConfig:
    def __init__(self, profile: Dict[str, Any]):
        self.name = profile.get("name", "wind-turbine")
        self.count = int(profile.get("count", 1))
        self.containers_per_node = int(profile.get("containers_per_node", 6))
        self.node_template = profile.get("node_template", {})
        self.container_template = profile.get("container_template", {})
        self.templates = profile.get("templates", {})
        self.ground_truth = profile.get("ground-truth-module", {})
        self.helics = profile.get("helics", {})
        self.labels = profile.get("labels", {})

        ext_net_config = self.container_template.get("external_network", {})
        if isinstance(ext_net_config, list) and ext_net_config:
            self.ext_net = ext_net_config[0]
        else:
            self.ext_net = ext_net_config
...
class WindTurbine(ScalePlugin):
    ...
    def _resolve_ext_start_ip(self) -> ipaddress.IPv4Address:
        ...
        return ipaddress.IPv4Address("192.168.100.1")

    def _process_helics_broker_metadata(self, md: Dict[str, Any]) -> Optional[str]:
        if "broker" in md:
            broker = md["broker"]
    ...
    def pre_configure(self, app: AppBase, profile: Dict[str, Any]) -> None:
        ...
        # Allow overriding the entire network block from the profile
        if "network" in self.config.node_template:
            self.base_spec["network"] = self.config.node_template["network"]

        logger.log(
            "DEBUG", f"WindTurbine plugin configured for {self.config.count} nodes."
        )
